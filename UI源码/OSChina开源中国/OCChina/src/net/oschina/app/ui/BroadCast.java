...
import net.oschina.app.ui.mainview.MainView;
...
public class BroadCast extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		{	
			...
			//动态-总数
			if(MainView.bv_active != null){
				if(activeCount > 0){
					MainView.bv_active.setText(activeCount+"");
					MainView.bv_active.show();
				}else{
					MainView.bv_active.setText("");
					MainView.bv_active.hide();
				}
			}
			//@我
			if(MainView.bv_atme != null){
				if(atmeCount > 0){
					MainView.bv_atme.setText(atmeCount+"");
					MainView.bv_atme.show();
				}else{
					MainView.bv_atme.setText("");
					MainView.bv_atme.hide();
				}
			}
			//评论
			if(MainView.bv_review != null){
				if(reviewCount > 0){
					MainView.bv_review.setText(reviewCount+"");
					MainView.bv_review.show();
				}else{
					MainView.bv_review.setText("");
					MainView.bv_review.hide();
				}
			}
			//留言
			if(MainView.bv_message != null){
				if(msgCount > 0){
					MainView.bv_message.setText(msgCount+"");
					MainView.bv_message.show();
				}else{
					MainView.bv_message.setText("");
					MainView.bv_message.hide();
				}
			}
