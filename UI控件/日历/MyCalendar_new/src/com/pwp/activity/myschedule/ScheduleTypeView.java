...
public class ScheduleTypeView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(int i = 0 ; i < AppParams.sch_type.length ; i++){
			radio = new RadioButton(this);
			if(i == sch_typeID){
				radio.setChecked(true);
			}
	        radio.setText(AppParams.sch_type[i]);
	        radio.setId(i);
		...
		group.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				.setSingleChoiceItems(
						new String[] { AppParams.remind[0], AppParams.remind[1], AppParams.remind[2], AppParams.remind[3], AppParams.remind[4], AppParams.remind[5], AppParams.remind[6], AppParams.remind[7] }, remindID,
